...
async function runValueInvestingCommittee(prompt) {
  const skillDir = '/Users/lisa/agent-skills/value-investing-committee';
  const scriptPath = path.join(skillDir, 'scripts', 'run_committee.py');
  if (!fs.existsSync(scriptPath)) throw new Error(`Value investing skill not found: ${scriptPath}`);
...
async function getTushareSelectionMarketRows() {
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    try {
      ...
      const [basicRows, valuationRows] = await Promise.all([
        tushare('stock_basic', { list_status: 'L' }, 'ts_code,name,industry,list_status'),
        tushare('daily_basic', { trade_date: tradeDate }, 'ts_code,trade_date,close,turnover_rate,volume_ratio,pe,pe_ttm,pb,total_mv,circ_mv'),
      ]);
